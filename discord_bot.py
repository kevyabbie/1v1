...
HOST = os.getenv("HOST", "0.0.0.0")  # Use 0.0.0.0 for Railway
...
class DiscordBot:
    ...
    def _setup_events(self):
        ...
        async def on_ready():
            ...
            # Start socket server in background thread (optional for Railway)
            # Only start if not in production or if explicitly enabled
            if os.getenv("ENABLE_SOCKET", "false").lower() == "true":
                ...
                logger.info("Socket server disabled (set ENABLE_SOCKET=true to enable)")
