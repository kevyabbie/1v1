...
Allows authorized user to add ghost players to their party for testing
Only accessible by user ID: 822110342724190258
"""
...
def setup_ghost_player_commands(tree: app_commands.CommandTree, ghost_system: GhostPlayerSystem):
    ...
    async def ghost_add(interaction: discord.Interaction):
        ...
        if success:
            ...
            if party:
                members_list = "\n".join([
                    f"{i+1}. {m.display_name} {'👑' if i == 0 else ''} {'👻' if hasattr(m, 'display_name') and m.display_name.startswith('Ghost_') else ''}"
    ...
    async def ghost_clear(interaction: discord.Interaction):
        ...
        if success:
            ...
            if party:
                members_list = "\n".join([
                    f"{i+1}. {m.display_name} {'👑' if i == 0 else ''}"
                    for i, m in enumerate(party.members)
                ])
                embed.add_field(
                    name=f"Party Members ({party.get_size()}/{party.max_size})",
